...
import {
  Grid, Ruler, FileSpreadsheet, Layers, BoxIcon, TrendingUp, Wrench,
  ArrowLeft, CheckCircle2, Star, PlayCircle, Zap
} from 'lucide-react';
  ...
  <nav className="glass-nav sticky top-0 z-50 border-b border-slate-200/60 px-6 py-4">
    <div className="max-w-7xl mx-auto flex justify-between items-center">
      ...
      <div className="flex items-center gap-6">
        <Link to={createPageUrl('ModulesDashboard')} className="hidden sm:block text-sm font-bold text-slate-700 hover:text-indigo-600">
          Login
        </Link>
        <Link to={createPageUrl('ModulesDashboard')} className="px-6 py-3 rounded-full font-bold text-sm bg-gradient-to-r from-indigo-500 to-purple-500 hover:brightness-110 text-white shadow-lg shadow-indigo-500/30 transition-all duration-300">
          Teste Grátis
        </Link>
      </div>
    </div>
...
export default function VidracariaInfo() {
  const module = {
    name: "VidroSmart",
    tagline: "Engenharia de precisão para vidraçarias",
    description: "Chega de erros de cálculo. Gere orçamentos técnicos precisos, listas de corte e otimize o aproveitamento das chapas de vidro.",
    color: "from-purple-500 to-indigo-600",
    textColor: "text-purple-600",
    image: "/images/vidrosmart_hero.png",
    gallery: [
      "/images/vidrosmart_gallery.png",
      "/images/vidrosmart_gallery.png",
      "/images/vidrosmart_gallery.png"
    ],
    stats: [
      { value: "0%", label: "Erros de Cálculo" },
      { value: "x3", label: "Velocidade Orçamento" },
      { value: "+20%", label: "Lucratividade" }
    ],
    testimonial: {
      text: "Antes eu demorava horas para calcular folgas e ferragens. Com o VidroSmart, faço o orçamento na frente do cliente em 5 minutos.",
      author: "Carlos Eduardo, Vidraçaria Transparência"
    },
    deepDive: [
      {
        title: "Projetista 3D Automático",
        desc: "Insira as medidas do vão e o sistema calcula automaticamente as folgas do vidro, quantidade de perfis e ferragens necessárias baseadas na norma técnica.",
        image: "/images/vidrosmart_gallery.png"
      },
      {
        title: "Gestão de Obras e Instalação",
        desc: "Acompanhe o status de cada pedido, da têmpera até a instalação final. Emita ordens de serviço digitais para sua equipe de campo.",
        image: "/images/vidrosmart_gallery.png"
      }
    ],
    benefits: [
      { icon: Ruler, title: "Cálculo de Folgas", desc: "Configuração automática para Blindex e esquadrias." },
      { icon: FileSpreadsheet, title: "Orçamentos em PDF", desc: "Propostas técnicas com desenho e foto dos produtos." },
      { icon: Layers, title: "Otimização de Corte", desc: "Melhor aproveitamento de chapas e perfis." },
      { icon: BoxIcon, title: "Estoque de Chapas", desc: "Controle por m², peso e espessura." },
      { icon: TrendingUp, title: "Precificação Dinâmica", desc: "Atualização automática baseada no custo do material." },
      { icon: Wrench, title: "Ordem de Serviço", desc: "Checklist de instalação e aceite digital do cliente." }
    ],
    features: ["Calculadora de Projetos", "PDF Profissional", "Controle de Têmpera", "Gestão de Obras", "Estoque de Alumínio", "Financeiro", "App do Instalador", "NFS-e"]
  };
        ...
        <div className="max-w-7xl mx-auto w-full relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to={createPageUrl('ModulesDashboard')} className="text-lg px-8 py-5 rounded-2xl shadow-xl shadow-indigo-500/20 bg-gradient-to-r from-indigo-500 to-purple-500 hover:brightness-110 text-white font-bold text-center transition-all">
          ...
          {module.deepDive.map((item, idx) => (
            <div key={idx} className={`flex flex-col ${idx % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-16`}>
              ...
              <div className="lg:w-1/2">
                ...
                <button className="flex items-center gap-2 text-purple-600 hover:gap-4 transition-all font-bold">
